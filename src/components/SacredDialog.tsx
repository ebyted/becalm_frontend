    ...
    <div className="glass-card dialogo-sagrado" style={{
      ...
      <div className="mensajes" style={{margin:'1.5em 0',minHeight:'120px',display:'flex',flexDirection:'column',gap:'0.8em',padding:'0.5em 0'}}>
        {messages.map((msg: any, i: number) => {
          // Paleta: #FFFFEF (fondo), #7B8057 (verde), #E5E5D7 (beige), #4A4D34 (oscuro)
          const userBg = '#E5E5D7'; // Emisor (usuario)
          const userColor = '#4A4D34';
          const guiaBg = '#FFFFEF'; // Receptor (guía)
          const guiaColor = '#7B8057';
          return (
            <div
              key={i}
              style={{
                ...
                maxWidth: '80%',
                background: msg.from === 'usuario' ? userBg : guiaBg,
                color: msg.from === 'usuario' ? userColor : guiaColor,
